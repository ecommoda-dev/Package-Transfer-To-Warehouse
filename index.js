...
//    `٢٠` → `٤٠` (v1.3.2، 19-09-2026 · قرار أحمد): القياس التاني بعد الرفع
//    الأول قال `s1Cancelled` **لسه بتتقصّ حتى عند ٢٠ صفحة** (٥٬٠٠٠ صف) —
//    يعني عدد `Cancelled` من ٢٠٢٦-٠٤-٠١ أكتر من ٥٬٠٠٠. اترفع للضعف (`٤٠` =
//    ١٠٬٠٠٠ صف) كخطوة تانية — والتكلفة لسه رخيصة نسبيًا (~٣٢ نقطة/نداء).
// ⚠️ **`queryCost.step1Detail`** بيقول لكل استعلام من التلاتة كام صفحة
//    استخدم واتقصّ ولا لأ — لو `truncated: true` رجعت تاني بعد الرفع ده،
//    الرقم ده هو اللي هيقول أنهي استعلام بالظبط لسه محتاج سقف أعلى.
// 🔴 **ولو الرفع ده لسه مش كفاية:** الحل مش رفع الرقم تالت مرة — المشكلة
//    الجذرية إن `Cancelled` حالة بتتراكم من غير سقف طبيعي واضح، ورفع الصفحات
//    مايتقاربش (converge) معاها. البديل الحقيقي وقتها هو تضييق نطاق الأرضية
//    الزمنية (`ORDERS_SINCE`) نفسها — قرار منفصل، مش سقف صفحات.
const STEP1_MAX_PAGES = 40;    // حارس حلقة — ٢٥٠×٤٠ = ١٠٬٠٠٠ لكل استعلام
